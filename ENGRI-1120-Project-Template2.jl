...
md"""
Before describing the methods used to procure the results the analysis, it is important to establish the assumptions that were used throughout the design of the process. Thus, a list of assumptions follows:
...
Initially, the overall reaction for the process needed to be compiled. Using the consitituent reactions in the table below, the overall reaction of the process was determined to be: 
...
begin
	...
	with_terminal() do
		...
		state_table = ["Maltose + H2O <=> 2 D-Glucose", "Xylitol + NAD+ <=> D-Xylulose + NADH + H+","ATP + alpha-D-Glucose <=> ADP + alpha-D-Glucose 6-phosphate -","alpha-D-Glucose 6-phosphate <=> beta-D-Fructose 6-phosphate","ATP + beta-D-Fructose 6-phosphate <=> ADP + beta-D-Fructose 1,6-bisphosphate -","beta-D-Fructose 1,6-bisphosphate <=> Glycerone phosphate + D-Glyceraldehyde 3-phosphate","D-Glyceraldehyde 3-phosphate <=> Glycerone phosphate","D-Glyceraldehyde 3-phosphate + Orthophosphate + NAD+ <=> 3-Phospho-D-glyceroyl phosphate + NADH + H+"]

		# display -
		println("Stuff")
		state_table_header_row = (["Reaction                      "]);
...
By chaining multiple such separation units together, a desired degree of separation was achieved. Specifically, it was found that 4 units were enough to produce an output stream with 98.0713% purity, as can be seen in the Figure immidiately below:."""
...
The price of PGDN was estimated through the equation: 1 * 56.50 / 1000 + 2.65 * 57 / 100, where (56.50 $ / 1000g) was the cost of 1,2 propanediol and (57 $ / 100g) was the cost of potassium nitrate.
...
# ╔═╡ 15332dee-b766-4ef9-b00a-200a071c86b8
md"""
As can be seen from the table directly above, the end product of PGDN is produced at a rate of 1.05713 grams per hour.
"""
